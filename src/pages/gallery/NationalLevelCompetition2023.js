...
const NationalLevelCompetition2023 = () => {
  const [images, setImages] = useState([]);

  useEffect(() => {
    const fetchImages = async () => {
      const snapshot = await getDocs(collection(db, "gallery2023NationalLevel"));
    ...
    <div style={{ background: "#fff8f0" }}>
      ...
      <Container className="py-5">
        <h2 className="mb-2 fw-bold text-orange text-center" style={{ fontSize: "2.2rem" }}>
          National Level Competition 2023 Gallery
        </h2>
        <Row>
          {images.map((img, idx) => (
...
export default NationalLevelCompetition2023;
