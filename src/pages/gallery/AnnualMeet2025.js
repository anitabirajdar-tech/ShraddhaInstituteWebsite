...
const AnnualMeet2025 = () => {
  const [images, setImages] = useState([]);

  useEffect(() => {
    const fetchImages = async () => {
      try {
        const snapshot = await getDocs(collection(db, "gallery2025AnnualMeet"));
  ...
  return (
    <div style={{ background: "#fff8f0" }}>
      <Container className="py-5">
        <h2 className="mb-2 fw-bold text-orange text-center">Annual Meet 2025 Gallery</h2>
        <Row>
          {images.map((img, idx) => (
